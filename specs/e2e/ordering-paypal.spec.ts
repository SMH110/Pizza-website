...
describe("PayPal", () => {
    ...
    it("I can add 3 pizzas, a side and a drink to the basket and check out using PayPal", async () => {
        ...
        expect(await orderSummary.getAddress()).to.equal('1 The Street, Some Road, Foo Town, CR7 2GB');
        ...
        expect(await orderDetails.orderNotes.getText()).to.equal('Some test PayPal order notes');
        ...
        expect(await orderItems[1].name.getText()).to.equal('Garlic Bread - Only garlic');
