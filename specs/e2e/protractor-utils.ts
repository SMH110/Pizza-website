...
export async function urlShouldBecome(predicate: (url: string) => boolean) {
  ...
  try {
    ...
    console.log(
      `urlShouldBecome failed. Last URL was ${lastUrl}. Looking for ${predicate.toString()}.`
    );
    throw e;
  }
}
...
export async function whenVisible<T>(
  locator: By,
  action: (element: ElementFinder) => T
): Promise<T>;
export async function whenVisible<T>(
  locator: By,
  action?: (element: ElementFinder) => T
): Promise<T | ElementFinder> {
...
export async function whenVisibleAndNotMoving(
  locator: By
): Promise<ElementFinder>;
export async function whenVisibleAndNotMoving<T>(
  locator: By,
  action: (element: ElementFinder) => T
): Promise<T>;
export async function whenVisibleAndNotMoving<T>(
  ...
  return whenVisible(locator, async element => {
    console.log(
      "whenVisibleAndNotMoving - Element visible... waiting for element to stop moving"
    );
    ...
    while (true) {
      ...
      if (
        ...
        if (attempts++ === 30) {
          throw new Error(
            "Element was moving for longer than 3 seconds" + locator.toString()
          );
        }
      }
    }
...
export async function whenClickable<T>(
  locator: By,
  action: (element: ElementFinder) => T
): Promise<T>;
export async function whenClickable<T>(
...
export async function whenAnyVisible<T>(
  locator: By,
  action: (element: ElementArrayFinder) => T
): Promise<T>;
export async function whenAnyVisible<T>(
  locator: By,
  action?: (element: ElementArrayFinder) => T
...
export async function doInsideIFrame<T>(locator: By, action: () => T) {
  ...
  await browser.driver.wait(
    async () => (await browser.driver.findElements(locator)).length > 0,
    UI_READY_TIMEOUT
  );
