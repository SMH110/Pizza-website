...
class OrdersPageObject {
  async getOrderByName(firstName: string, lastName: string) {
    ...
    let orderSummary = await whenAnyVisible(
      by.className("order-summary"),
      // TODO - Remove the any cast here
      orderSummaries =>
        orderSummaries
          .filter((async (summary: ElementFinder) =>
            (await summary.element(by.className("name")).getText()) ===
            fullName) as any)
          .first()
    );
...
class OrderDetailsObject {
  ...
  async getOrderItems() {
    ...
    let orderItems = await element.all(
      by.css(`#details_${this.orderId} .order-item`)
    );
