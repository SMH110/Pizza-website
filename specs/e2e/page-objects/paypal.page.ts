...
class PayPalPageObject {
  ...
  async payNew() {
    ...
    await doInsideIFrame(by.name("injectedUl"), async () => {
      await whenVisible(by.id("email"), email =>
        email.sendKeys("csharpandsons-buyer@gmail.com")
      );
    ...
    await whenNotPresent(by.id("spinner"));
    await whenClickable(by.buttonText("Continue") as any, payNow =>
      payNow.click()
    );
  ...
  async payLegacy() {
    ...
    await whenVisible(by.id("login_email"), email =>
      email.sendKeys("csharpandsons-buyer@gmail.com")
    );
