...
describe("EPDQ e2e test", () => {
  ...
  it("I can add a pizza to the basket and check out using Barclays ePDQ", async () => {
    ...
    expect(await orderSummary.getAddress()).to.equal(
      "1 The Street, Foo Town, CR7 2GB"
    );
    expect(await orderSummary.getPhone()).to.equal("01234567890");
    expect(await orderSummary.getTotalPayment()).to.equal(
      "£15.49 (Credit / Debit Card)"
    );
    ...
    expect(await orderItems[0].name.getText()).to.equal(
      "Spinaci - Extra Large"
    );
