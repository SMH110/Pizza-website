...
export const _2ExtraLargePizzas: DiscountRule = {
  name: "Any 2 Extra Large pizzas for £16.99",
  description:
    "Any 2 x Extra Large pizzas (collection only, or add a side for delivery) for £16.99. Dips are not counted as a side for this discount.",
  calculate(basket: DiscountCalculationDto) {
    ...
    let timesToApply =
      basket.deliveryMethod === "Collection"
        ? Math.floor(pizzas.length / 2)
        : Math.min(Math.floor(pizzas.length / 2), sides.length);
    return sum(pizzas.slice(0, timesToApply * 2)) - timesToApply * 16.99;
  }
};
