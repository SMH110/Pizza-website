...
export const godfatherSpecialMealDealLarge: DiscountRule = {
  ...
  description:
    "1 Large pizza, 1 x Spicy Wings (8 pieces) or BBQ Wings, and 1 x Bottle of Soft Drink for £13.99",
...
export const godfatherSpecialMealDealExtraLarge: DiscountRule = {
  ...
  description:
    "1 Extra Large pizza, 1 x Spicy Wings (8 pieces) or BBQ Wings, and 1 x Bottle of Soft Drink for £16.99",
  calculate(basket: DiscountCalculationDto) {
...
function calculate(
  ...
  let timesToApply = Math.min(
    Math.floor(pizzas.length),
    Math.floor(spicyWings.length),
    Math.floor(drinks.length)
  );
  return (
    sum(pizzas.slice(0, timesToApply)) +
    sum(spicyWings.slice(0, timesToApply)) +
