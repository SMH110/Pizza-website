export function validateOrderRequest(request: PlaceOrderRequestValidationDto, availablePaymentMethods: string[]): string[] {
    let errors: string[] = [];
...
function validateBasket(request: PlaceOrderRequestValidationDto): string[] {
    let errors: string[] = [];
...
function validateBuyerDetails(request: PlaceOrderRequestValidationDto): string[] {
    let errors: string[] = [];
    if (!request.buyer) {
        errors.push('Buyer details are required')
    }
...
function validateDeliveryAddress(request: PlaceOrderRequestValidationDto): string[] {
    let errors: string[] = [];
...
interface PlaceOrderRequestValidationDto extends PlaceOrderRequest {
    orderItems: BasketItem[] | OrderLineItem[];
}