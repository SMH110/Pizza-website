...
export abstract class BasketService {
    ...
    return (
      discounts
        .filter(
          x =>
            this.discountCode
              ? x.discountCode === this.discountCode.toUpperCase()
              : x.discountCode === undefined
        )
        .map(x => ({
          name: x.name,
          amount: this.normalise(
            x.calculate({
  ...
  getTotalPayable(): number {
    ...
    return this.normalise(
      this.getTotalPrice() - discountAmount - voucherAmount
    );
  }
  ...
  static getDescription(item: BasketItem) {
    return [item.name, item.version, item.options.join(", ")]
      .filter(x => !!x)
      .join(" - ");
  }
}
