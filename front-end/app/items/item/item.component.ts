...
export class ItemComponent implements OnInit {
  ...
  constructor(
    ...
    private itemNotificationService: NotificationService,
  ...
  async addToBasket() {
    ...
    this.basket.addToBasket(
      Object.assign(
        { version: this.selectedVersion, quantity: 1, options },
        this.item
      )
    );
  ...
  private async getOptions() {
    if (this.item.tags.indexOf("pizza") !== -1) {
      return this.modalService.open(AddPizzaModalComponent, {
        data: { item: this.item, version: this.selectedVersion }
      });
    } else if (this.item.name === "Potato Skins") {
      return this.modalService.open(PotatoSkinsModalComponent, {
        data: { item: this.item }
      });
    } else {
      return [];
