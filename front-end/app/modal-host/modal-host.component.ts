import { Component, ViewChild, ComponentFactoryResolver, OnDestroy, ViewContainerRef } from '@angular/core';
...
export class ModalHostComponent implements OnDestroy {
    ...
    constructor(private componentFactoryResolver: ComponentFactoryResolver, private modalService: ModalService) {
        this.modalRequestedSubscription = this.modalService.modalRequested.asObservable()
            .subscribe(modalRequest => this.openModal(modalRequest));
    }

    private async openModal(modalRequest: ModalRequest<any, any>) {
        let componentFactory = this.componentFactoryResolver.resolveComponentFactory(modalRequest.componentType);
        let componentRef = this.placeholder.createComponent<BaseModalComponent<any, any>>(componentFactory);
