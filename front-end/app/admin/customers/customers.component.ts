...
export class CustomersComponent {
  ...
  private async initialise() {
    ...
    try {
      ...
      let customers = Object.keys(ordersByEmailAddress).map<Customer>(x => ({
        email: x,
        orders: ordersByEmailAddress[x],
        vouchers: []
      }));
      ...
      this.handleError(
        error,
        "There was an unexpected error refreshing the customers. Please try again."
      );
    }
  }
  ...
  async sendVoucher(customer: Customer) {
    ...
    if (this.voucherAmount < this.MIN_VOUCHER_AMOUNT) {
      this.errorService.displayErrors([
        `You can't send a voucher less than £${this.MIN_VOUCHER_AMOUNT}`
      ]);
      return;
    }
    if (this.voucherAmount > this.MAX_VOUCHER_AMOUNT) {
      this.errorService.displayErrors([
        `You can't send a voucher more than £${this.MAX_VOUCHER_AMOUNT}`
      ]);
      return;
    }
    ...
    try {
      await this.adminService.createVoucher({
      ...
      this.handleError(
        error,
        "There was an error creating the voucher. Please try again."
      );
    }
  }
  ...
  getNames(customer: Customer) {
    return distinct(
      customer.orders.map(x => `${x.buyer.firstName} ${x.buyer.lastName}`)
    );
  }
  ...
  getCurrentVouchers(customer: Customer) {
    return customer.vouchers.filter(
      x => x.dateUsed === null && new Date() < new Date(x.expiryDate)
    ).length;
  }
