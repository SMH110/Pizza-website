...
export class ReportingComponent {
  ...
  private async initialise() {
    try {
      let orders = (await this.adminService.getOrders()).filter(
        x => x.status === "Complete"
      );
      this.monthlyData = getPeriodData(
        orders,
        x =>
          moment(x.date)
            .startOf("month")
            .toDate(),
        x => moment(x).format("MMM YY")
      );
      this.bestMonthlyTotalOrderValue = this.monthlyData
        .slice()
        .sort(
          (a, b) => b.totalOrderValue - a.totalOrderValue
        )[0].totalOrderValue;
      this.weeklyData = getPeriodData(
        orders,
        x =>
          moment(x.date)
            .startOf("isoWeek")
            .toDate(),
        x => moment(x).format("DD MMM YY")
      );
      this.bestWeeklyTotalOrderValue = this.weeklyData
        .slice()
        .sort(
          (a, b) => b.totalOrderValue - a.totalOrderValue
        )[0].totalOrderValue;
      ...
      this.handleError(
        error,
        "There was an unexpected error retrieving the orders."
      );
    }
  }
...
function getPeriodData(
  ...
  return Array.from(periodData.values()).sort(
