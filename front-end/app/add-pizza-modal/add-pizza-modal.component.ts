...
import Toppings, {
  PizzaToppingPrices
} from "../../../shared/static-data/toppings";
...
export class AddPizzaModalComponent extends BaseModalComponent<
  AddPizzaModalDto,
  string[]
> {
  ...
  addSelectedTopping() {
    let existingTopping = this.selectedToppings.find(
      x => x.name === this.toppingToAdd.name
    );
  ...
  removeTopping(selectedTopping: SelectedTopping) {
    this.selectedToppings.splice(
      this.selectedToppings.indexOf(selectedTopping),
      1
    );
  }
  ...
  getBaseName() {
    return this.data.item.name === "Garlic Meat Lover"
      ? "Garlic sauce"
      : "Tomato sauce";
  }
