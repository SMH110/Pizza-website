...
export const IsBarclaysEPDQEnabled =
...
export default class BarclaysEPDQ implements PaymentGateway {
  ...
  public async createPaymentRedirect(
    ...
    let redirectParameters = getPaymentRedirectParameters(
      order,
      this.baseReturnAddress
    );
  ...
  application.get("/barclays-epdq/feedback", async (req, res) => {
    try {
      ...
      if (!SHASIGN || SHASIGN.length === 0) {
        throw new Error(
          `Missing SHASIGN for ${JSON.stringify(req.query, null, 4)}`
        );
      }
      ...
      if (getSHASIGN(feedback, process.env.BARCLAYS_EPDQ_SHA_OUT) !== SHASIGN) {
        throw new Error(
          `SHASIGN check failed for ${JSON.stringify(req.query, null, 4)}`
        );
      }

      console.log(
        `Barclays ePDQ feedback received. Locating order ${feedback.ORDERID}.`
      );
      ...
      if (status === "ACCEPTED") {
        if (order.status === "Awaiting Payment") {
          ...
          console.log(
            `Payment accepted for order. Order ${feedback.ORDERID} updated.`
          );
        ...
        await order.save();
        console.log(
          `Refund complete for order. Order ${feedback.ORDERID} updated.`
        );
      } else if (status === "CANCELLED") {
        console.log(
          `Transaction status ${status} for ${JSON.stringify(
            req.query,
            null,
            4
          )}`
        ...
        await order.save();
        console.log(
          `Payment NOT accepted for order. Order ${feedback.ORDERID} updated.`
        );
        throw new Error(
          `Barclays ePDQ payment not accepted. Transaction status ${status} for ${JSON.stringify(
            req.query,
            null,
            4
          )}`
...
function getPaymentRedirectParameters(
  order: PersistedOrder,
  baseReturnAddress: string
): BarclaysEPDQPaymentRedirectRequest {
  ...
  let paymentRequest = {
    ...
    OWNERADDRESS: [order.billingAddress.line1, order.billingAddress.line2]
      .filter(x => !!x)
      .join(", "),
