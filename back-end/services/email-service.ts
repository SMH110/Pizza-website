...
export async function sendOrderConfirmedEmail(
  ...
  let readyInText = moment(Date.now())
    .add(readyInMinutes, "minutes")
    .fromNow();
  let expiresOn = moment(voucher.expiryDate).format("dddd Do MMM [at] HH:mm");
  let html = await renderEjsTemplate(__dirname + "/order-confirmed.ejs", {
    order,
    voucher,
    readyInMinutes,
    readyInText,
  ...
  await sendEmail(
    order.buyer.email,
    "Your order has been confirmed + 10% off when you next order!",
    html
  );
}
...
export async function sendVoucherCode(voucher: Voucher) {
  ...
  let expiresOn = moment(voucher.expiryDate).format("dddd Do MMM [at] HH:mm");
  let html = await renderEjsTemplate(__dirname + "/voucher-code.ejs", {
    voucher,
    expiresIn,
    expiresOn
  });
...
export async function sendVoucherReminder(voucher: Voucher) {
  ...
  let expiresIn = moment(voucher.expiryDate).fromNow();
  let expiresOn = moment(voucher.expiryDate).format("dddd Do MMM [at] HH:mm");
  let html = await renderEjsTemplate(__dirname + "/voucher-reminder.ejs", {
    voucher,
    expiresIn,
    expiresOn
  });
...
async function sendPlaceOrderEmailToStore(order: Order) {
  let subject = `New order placed - £${order.totalPayment} - ${
    order.buyer.firstName
  } ${order.buyer.lastName}`;
  let html = await renderEjsTemplate(__dirname + "/order-placed.ejs", {
    order,
    isStoreCopy: true,
    getDescription: BasketService.getDescription
  });
...
async function sendEmail(
  ...
  try {
    await transporter.sendMail({
      from: `'Godfather Pizza' <${STORE_EMAIL_ADDRESS}>`,
      to: recipientEmail,
      subject,
      html
    });
